...
const Experiencie = () => {
  ...
  return (
    <section ref={experienceRef} className={styles.container} id="experiencie">
      ...
      <motion.header className={styles.header} style={{ x: headerX, opacity: headerOpacity }}>
        <motion.h2 className={styles.title} style={{ y: titleY, opacity: titleOpacity }}>
          <SplitLines rootBlock lines={[{ text: 'Journal_' }]} delayChildren={0.08} />
        </motion.h2>
        <motion.div
          className={styles.path}
          style={{ x: pathX, opacity: pathOpacity }}
        >
      ...
      <motion.div
        ...
        style={{ x: listX, opacity: listOpacity }}
        initial="hidden"
      ...
      >
        {journalExperiences.map((exp, index) => (
          <JournalEntry key={exp.slug} exp={exp} index={index} />
        ))}
      </motion.div>
    </section>
  )
...
const JournalEntry = ({ exp, index }: { exp: JournalExperience; index: number }) => {
